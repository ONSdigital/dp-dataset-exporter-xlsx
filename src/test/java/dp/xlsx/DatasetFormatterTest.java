...
public class DatasetFormatterTest {
	...
	public void format() throws IOException {
		...
		try (final InputStream stream = V4FileTest.class.getResourceAsStream("v4_0.csv")) {
			...
			final DatasetFormatter datasetFormatter = new DatasetFormatter(workBookStyles, sheet, file,
					datasetMetadata);
	...
	@Test
	public void format_WithDatasetDimensionLabel() throws IOException {
		String expectedLabel = "Mordor";		
		
		CodeList cl = new CodeList();
		cl.setName("Geography");
		cl.setLabel(expectedLabel);
		Collection<CodeList> collection = new ArrayList<CodeList>();
		collection.add(cl);

		datasetMetadata.setDimensions(collection);
		...
		assertThat(sheet.getRow(2).getCell(0).getStringCellValue()).isEqualTo(expectedLabel);
	}
	...
	public void format_ZeroDecimal() throws IOException {
		...
		// Given some v4 file data with an observation that has a zero decimal place
		// (88.0)
